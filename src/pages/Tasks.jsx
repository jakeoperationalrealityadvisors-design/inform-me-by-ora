...
export default function Tasks() {
    const { isAdmin, canViewAll, canCreateTasks, user } = useUserRole();
    const { t } = useLanguage();
    ...
    const { data: tasks = [], isLoading } = useQuery({
        queryKey: ['tasks', search, selectedCategory, statusFilter],
        queryFn: async () => {
            let query = {};
            if (search) {
            ...
            return await httpClient.entities.Task.list('-created_date', 100, query);
        }
    });
    
    const filteredTasks = tasks.filter(task => 
        canViewAll || task.created_by === user?.email || task.assigned_to === user?.email
    );
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
                {isLoading ? (
                    ...
                    <EmptyState
                        ...
                        description={search ? "Try adjusting your search terms" : "Create your first task to get started"}
                        actionLabel="Create Task"
                        onAction={() => navigate(createPageUrl('CreateTask'))}
                    />
                ) : (
